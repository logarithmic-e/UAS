...
if img is None:
    raise FileNotFoundError("Could not load IMG")
...
level0_mask = cv.bitwise_and(cv.inRange(hsv, (30, 40, 180), (80, 255, 255)),mask)   # ground
level1_mask = cv.bitwise_and(cv.inRange(hsv, (30, 40, 100), (80, 255, 180)),mask)   # slope
level2_mask = cv.bitwise_and(cv.inRange(hsv, (30, 40, 0),   (80, 255, 100)),mask)   # peak
...
def elevation_detect(x, y):
    ...
    return elevation_dict.get(elevation, elevation_dict["ground"])
...
for cnt in contours:
    ...
    elevation = elevation_detect(x_mid, y_mid)

    cord_of_shapes.append({
        'color': color,
        'shape': shape_identifier(cnt),
    ...
    cv.putText(annotated_img, label, (text_x, text_y),
               cv.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 0), 2, cv.LINE_AA)
...
#https://github.com/jtris/maze-solver/blob/main/src/maze_solver.py(uses cython so ignored)

path = []
start_point = None
end_point = None
for item in cord_of_shapes:
    if item['color'] == 'orange' and item['shape'] == 'Triangle':
        start_point = item['coords']
    elif item['color'] == 'purple' and item['shape'] == 'Triangle':
        end_point = item['coords']
    # elif item['color'] >= 0:
    #     path.append(item)

path.sort(
    key=lambda item: (item['color_score'], item['priority_score']),
    reverse=True
)
if start_point:
    path.append(start_point)

for casualty in cord_of_shapes:
    path.append(casualty['coords'])
...
final_filename = filename+'_'+'masked.jpg'

cv.imwrite(final_filename , annotated_img)
print (counter)
